...
pub async fn sql_ports_from_env() -> Result<(Option<PortSet>, L3Availability)> {
    ...
    let ports = PortSet {
        ...
        // The real sql implementation, so the conversation contract cases
        // actually exercise Postgres — the transactional lock and the sequence
        // allocator are precisely the parts an in-memory mutex cannot stand in for.
