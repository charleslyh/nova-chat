...
> 版本：v1.0
> 状态：**基线**（参数已推导完成，见 [`parameters.md`](./parameters.md)）
...
进入设计前，确认每条需求均有可执行的验收方式：

| 类别 | 条目数 | 验收方式 |
|------|-------|---------|
| FR 功能 | 29（顶层 24 条，其中 FR-2 展开为 6 条子项） | 功能测试 + 场景测试 |
| CR 正确性 | 11 | **并发压力测试 + 故障注入测试**（不可仅靠功能测试） |
| CON 约束 | 1 | 代码审查 + 业务侧规范 |
| SR 规模 | 5 | 压测（含伸缩性对比） |
| OR 运维 | 6 | 故障演练 + 变更演练 |
| SEC 安全 | 6 | 渗透测试 + 越权测试 + 沙箱逃逸测试 |
