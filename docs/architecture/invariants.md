...
> 版本：v1.1
> 依据：[`spec.md`](../requirements/spec.md) · 澄清见 D18（[`decisions.md`](./decisions.md#d18-对话层在任务系统之上--session-日志与-turn-领取)）
...
| # | 不变量 | 支撑需求 | 违反后果 |
|---|--------|---------|---------|
| **INV-10** | **任务执行输出与 attempt/fence 的绑定身份是 `task_id`**，与用户、**连接**无关；不得用连接或 sticky 路由充当输出身份 | FR-11, FR-14, CR-4 | 无法隔离重试轮次，或多端无法稳定续订任务级观测 |
| **INV-11** | **全序号由单点分配**，进入同一逻辑有序流的事件必须可比较因果顺序 | CR-6 | 不同观测者看到相反的因果顺序 |
| **INV-12** | **客户端唯一需持久化的连接级观测状态是游标 `(stream_id, last_seq)`**；服务端**不保存连接粘性或连接级游标**。`stream_id` 在单任务观测场景为 `task_id`，在多轮对话场景为 `session_id`（见 D18） | FR-14 | 需要连接粘性路由，且换设备无法续订 |
| **INV-13** | **一切可作为重建起点的载体必须记录其对应序号**（快照、投影、归档、异步查询视图均须携带 `seq`） | CR-5, FR-23 | 增量起点不确定 ⇒ 重复渲染或静默丢事件；异步视图无法判断滞后程度与重建位点 |
| **INV-14** | **热层（或当前可读层）已不包含所请求序号时必须明确报错**，不得静默从可用位置续发；须引导客户端改走快照或冷层。**业务上有效 Session 日志不得因热层卸载而不可恢复** | CR-5, D18 | 静默缺口极难排查；或历史对话无法打开 |
| **INV-15** | **快照序号严格单调**，回退的快照写入必须被丢弃 | CR-5 | 增量起点回退，重复投递 |
| **INV-16** | **可合并事件与不可合并事件必须显式区分**：输出增量可丢弃中间态，生命周期与互动消息不可丢 | CR-5, FR-12 | 慢客户端场景下丢失关键事件 |

### 3.0 用语：连接会话 ≠ Conversation / Session（D18）

| 词 | 含义 | INV-12 |
|----|------|--------|
| **连接会话**（connection session） | TCP/WS/SSE 连接及其服务端粘性状态 | **禁止**作为真相或游标权威 |
| **Conversation / Session** | 用户可见的对话线程（领域资源：成员、锁、快照、可回放日志） | **允许且必需**（多轮 Agent 场景） |

多轮对话的客户端重放游标为 `(session_id, last_seq)`。Session 流上的事件仍须携带 `task_id` / `message_id` / `attempt`，以满足 CR-4 与 UI 分组；**领取粒度仍是 Turn=Task**，不是整段 Session（D18）。

> **INV-10 未废止**：它禁止「输出身份绑在连接上」。D18 把**产品重放流**的键定为 `session_id`，与「写入必须可归因到 `task_id`+attempt」同时成立。
