...
> 推导文，非实现依据。拍板见 [`architecture/decisions.md`](./architecture/decisions.md)（D8 / D11 / D18）与 [`architecture/invariants.md`](./architecture/invariants.md) §3。
...
**StreamChannel**（per-session 连续 seq 的可回放 WAL）替换 jsonls；**snapshot** 存折叠开屏态（含 `snapshot_seq`）。领取库与流分承载。产品适配器对比见 [`design/drafts/stream-channel-adapters.md`](./design/drafts/stream-channel-adapters.md)（Redis Streams vs JetStream；一期默认 Redis，D18）。
