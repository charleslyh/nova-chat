...
### 3.2 FR-4：领取的作用域（D23 的教训，D25 的现行答案）

拉取协议曾依赖一个从未写明的前提：**领取方即宿主节点**。内存后端各持自有账本，该前提免费成立；账本变共享后前提失效，而没有任何代码或断言表达它。
...
D23 当时的解法是把领取限定在本节点（L0 用例 `claim-locality`）。**D25 换掉了前提本身**：在途缓冲升级为共享中间件后，缓冲不再属于任何进程，「领取方 ≠ 缓冲持有者」不再是缺陷。故 `claim-locality` 已废除，代之以 L0 用例 **`global-claim`**：断言任意执行进程可领取任意 queued 生成，且并发领取不双领（attempt 栅栏仍然生效）。

`check-deps` 的两条门禁随之反转：拉取端点仍不得回归；而 `claim` **不得**携带 `NodeTag`、sql claim 语句**不得**出现 `node_tag` 谓词——否则队列中的生成会被搁死在没有执行端的节点上。
