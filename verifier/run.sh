...
"$BIN/nova-responses-mem-server" --listen 127.0.0.1:19000 --control-listen 127.0.0.1:19001 &
PIDS+=($!)
wait_port 19000
...
"$BIN/nova-responses-sweep" --heartbeat-ttl-ms 90000 --retain-after-terminal-ms 60000 &
PIDS+=($!)
...
"$BIN/nova-responses-gateway" --config testing/config/node-a.toml &
PIDS+=($!)
...
(cd verifier && python3 -m http.server "$WEB_PORT" --bind 127.0.0.1) &
PIDS+=($!)
