...
use mock_agentd::completions::{CompletionsOutcome, CompletionsRequest, FinishReason};
use mock_agentd::{MockAgentRunner, NoopToolExecutor, Scheduler, SchedulerError};
...
impl Scheduler for CapturingScheduler {
    ...
    async fn schedule(
        ...
        *self.seen.lock().expect("lock") = Some(request.clone());
        Ok(CompletionsOutcome::text("ok", nova_responses::Usage::new(1, 1)))
    }
}
...
impl Scheduler for InvalidOutcomeScheduler {
    ...
    async fn schedule(
        ...
        Ok(CompletionsOutcome {
            items: vec![nova_responses::ResponseItem::Message {
                role: nova_responses::Role::Assistant,
                content: vec![],
                id: None,
                status: None,
            }],
            usage: nova_responses::Usage::new(1, 1),
...
async fn start_with_config(raw_text: &str) -> Harness {
    ...
    let app = nova_responses_gateway::routes::router(app_state);
