...
overview: 对 nova-chat 工作区做分层重构：将 crates/core 的全部内容（领域+端口+协议）并入 crates/responses（conversation 作为 response 的子模块），把 HTTP 接入层（routes/sse/auth/shutdown）从 responses 剥离到 gateway，把内存验证替身归入 verify/mock，移除孤儿 reconnect 并修正 FR-35 覆盖错位与 INV-33 归属。
todos:
  - id: remove-reconnect
  ...
  - id: update-gates-validate
    content: 更新 workspace 清单与 xtask 硬编码门禁（check_deps、coverage_baseline、crate 路径与名称），跑通 just verify / coverage / check-deps 全绿
    status: pending
    dependencies:
      - split-gateway-http
      - extract-sweep-lib
...
对 nova-chat 工作区做一次大规模分层重构，将当前的「横切分层 + 生产/验证混杂」调整为「按领域特性组织 + 明确生产/验证边界」。

## 核心调整

- **core 并入 responses**：`crates/core`（领域类型 + 端口 + 协议子集）的全部内容合并进 `crates/responses`，conversation 作为 responses 的子模块（response 聚合的组成部分，非独立 crate）。
- **领域数据结构共置**：把横切的 `ids.rs` 按领域拆散到 response / conversation / shared 等模块；`ports` 端口层保留独立横切（DIP 抽象边界）。
- **能力层与 HTTP 接入层分离**：responses 只保留能力层（service，无 axum）；routes / sse / auth / shutdown / state / config 等强耦合 gateway 的 HTTP 接入层迁入顶层 `gateway`（lib + bin 装配壳）。
- **sweep 独立**：sweeper 维护逻辑从 responses 抽出到 `crates/sweep`（lib），sweep bin 退化为薄壳。
...
重构后呈现三层：`crates`（领域 + 能力 + 执行 + 维护）、`gateway`（HTTP 接入 + 装配壳）、`verify`（mock + 测试 + 编排 + web 手动验证）。产品闭包与验证闭包仅在 responses 处相交，验证替身（mem）与生产载体物理隔离。

## 技术栈

- Rust 2021 + Cargo workspace，复用现有 tokio / axum / tower-http / tracing / clap 依赖，不引入新依赖。
- 纯结构重构：不改变运行时行为、不改变 trait 语义、不引入新架构模式。

## 实现策略

自底向上、每步保持可编译，避免一次性大爆炸：

1. 先做独立的 reconnect 移除（含 conformance case、coverage baseline、invariants.md），单独验证。
...
4. 把 sweeper 从 responses 抽出到 sweep lib。
...
```mermaid
graph TD
    responses["crates/responses 领域+端口+协议+能力层"]
    ...
    responses --> agent_runtime
    responses --> sweep
    responses --> gateway
    responses --> mock_server
...
│   │       ├── conversation/     # ConversationId、Conversation、ConversationEvent
...
│   ├── agent-runtime/            # [KEEP] 执行运行时，依赖 responses
...
└── verify/
    ├── mock/
    │   ├── server/               # [MODIFY] 原 adapters-mem + mem-server
...
- **保持既有 D 决策**：D14（responses 不依赖 adapter/ingress）、D17（mem 是验证替身）、D25（能力层抽离）、D27/D28（conversation 与 response 聚合）不被破坏。