---
name: responses-subset-refactor
overview: 将 nova-sessions 重构为对齐 OpenAI Responses 协议封闭子集的生成服务：主资源收敛为单次生成，实现 store 开关与 previous_response_id 服务端上下文拼接，上下文库与账本上持久化高可用、在途事件缓冲留进程内存并配四项崩溃缓解；删除开屏快照、会话级订阅、热冷归档与镜像投影；同步完成改名、协议子集契约发布与文档验证体系重写。
todos:
  - id: docs-contract
    content: 新增 ADR D20/D21/D22 并重写 spec v3、parameters、invariants，锁定 FR/CR/INV/SEC 编号
    status: completed
  - id: protocol-subset
    content: 建立协议封闭子集类型层：封闭枚举、严格拒绝未知、限长限深、引用链接内网拦截
    status: completed
    dependencies:
      - docs-contract
  - id: core-domain
    content: 重写 core：ResponseId/TenantId、事件对齐、context 与 canonical，新增四端口并删除 snapshot 与 StreamChannel/MetaStore
    status: completed
    dependencies:
      - protocol-subset
  - id: mem-adapters
    content: 重写 mem 适配器：0 基有界环、生成账本、上下文库单锁走链、HMAC 校验，删除 snapshot 与 mirror
    status: completed
    dependencies:
      - core-domain
  - id: sql-adapter
    content: 新建 adapters-sql：迁移脚本、原子领取、递归走链、孤儿收口、租户清除与过期清理
    status: completed
    dependencies:
      - core-domain
  - id: gateway-api
    content: 重写 gateway：三种响应模式、两条转发路径、租户鉴权、sweeper、优雅停机与启动孤儿收口
    status: completed
    dependencies:
      - mem-adapters
      - sql-adapter
  - id: verification
    content: 重写 L0 契约为双适配器共用并新增链闭合性断言，更新裁判与场景，新增 L3 与规范 diff，同步 xtask 基线
    status: completed
    dependencies:
      - gateway-api
  - id: rename-sweep
    content: 用 [subagent:code-explorer] 清扫残留引用，完成改名及 config/justfile/deploy/mock-agent/sim 适配
    status: completed
    dependencies:
      - gateway-api
  - id: design-docs
    content: 重写设计文档并新增上下文链、完整性、可靠性、协议子集四篇，归档作废草稿并起新期计划
    status: completed
    dependencies:
      - verification
      - rename-sweep
