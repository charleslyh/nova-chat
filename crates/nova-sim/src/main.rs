mod supervisor;

use std::net::SocketAddr;
use std::sync::Arc;
use std::time::Duration;

use anyhow::Result;
use axum::extract::{Path, State};
use axum::http::StatusCode;
use axum::response::{Html, IntoResponse, Response};
use axum::routing::{delete, get, post};
use axum::{Json, Router};
use clap::Parser;
use serde::Deserialize;
use tokio::sync::Mutex;
use tower_http::cors::CorsLayer;
use tracing::info;

use supervisor::{
    next_free_port_hint, parse_port, RegionRole, RegionSpec, SharedSim, SimSupervisor,
};

#[derive(Debug, Parser)]
struct Args {
    #[arg(long, default_value = "127.0.0.1:19090")]
    listen: String,
}

#[tokio::main]
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info".into()),
        )
        .init();

    let args = Args::parse();
    SimSupervisor::ensure_bins()?;
    let sim: SharedSim = Arc::new(Mutex::new(SimSupervisor::new()?));

    let app = Router::new()
        .route("/", get(index))
        .route("/api/state", get(api_state))
        .route("/api/preset/three-region", post(api_preset))
        .route("/api/regions", post(api_add_region))
        .route("/api/regions/{id}", delete(api_remove_region))
        .route("/api/regions/{id}/start", post(api_start_region))
        .route("/api/regions/{id}/stop", post(api_stop_region))
        .route("/api/regions/{id}/workers", post(api_start_worker))
        .route("/api/workers/{id}", delete(api_stop_worker))
        .route("/api/tasks", post(api_submit))
        .route("/api/stop-all", post(api_stop_all))
        .layer(CorsLayer::permissive())
        .with_state(sim);

    let addr: SocketAddr = args.listen.parse()?;
    info!("Nova Sim console → http://{addr}");
    let listener = tokio::net::TcpListener::bind(addr).await?;
    axum::serve(listener, app).await?;
    Ok(())
}

async fn index() -> Html<&'static str> {
    Html(include_str!("../static/index.html"))
}

async fn api_state(State(sim): State<SharedSim>) -> impl IntoResponse {
    let mut g = sim.lock().await;
    Json(g.snapshot().await)
}

async fn api_preset(State(sim): State<SharedSim>) -> Response {
    let mut g = sim.lock().await;
    match g.load_preset_three_region() {
        Ok(()) => {
            drop(g);
            // wait home
            let g = sim.lock().await;
            if let Err(e) = g.wait_healthy("home", Duration::from_secs(15)).await {
                return err(StatusCode::GATEWAY_TIMEOUT, e.to_string());
            }
            Json(serde_json::json!({"ok": true})).into_response()
        }
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

#[derive(Debug, Deserialize)]
struct AddRegionBody {
    id: String,
    role: String,
    #[serde(default)]
    listen: Option<String>,
    #[serde(default)]
    home_upstream: Option<String>,
}

async fn api_add_region(
    State(sim): State<SharedSim>,
    Json(body): Json<AddRegionBody>,
) -> Response {
    let mut g = sim.lock().await;
    let snap = g.snapshot().await;
    let used: Vec<u16> = snap.regions.iter().filter_map(|r| parse_port(&r.listen)).collect();
    let port = next_free_port_hint(&used, 18080);
    let listen = body
        .listen
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| format!("127.0.0.1:{port}"));
    let role = match body.role.as_str() {
        "home" => RegionRole::Home,
        "edge" => RegionRole::Edge,
        _ => return err(StatusCode::BAD_REQUEST, "role must be home|edge".into()),
    };
    let mut home_upstream = body.home_upstream.filter(|s| !s.trim().is_empty());
    if matches!(role, RegionRole::Edge) && home_upstream.is_none() {
        home_upstream = snap
            .regions
            .iter()
            .find(|r| matches!(r.role, RegionRole::Home))
            .map(|r| r.listen.clone());
    }
    let spec = RegionSpec {
        id: body.id,
        role,
        listen,
        home_upstream,
    };
    match g.add_region(spec) {
        Ok(()) => Json(serde_json::json!({"ok": true})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

async fn api_remove_region(State(sim): State<SharedSim>, Path(id): Path<String>) -> Response {
    let mut g = sim.lock().await;
    match g.remove_region(&id) {
        Ok(()) => Json(serde_json::json!({"ok": true})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

async fn api_start_region(State(sim): State<SharedSim>, Path(id): Path<String>) -> Response {
    let mut g = sim.lock().await;
    match g.start_region(&id) {
        Ok(()) => {
            if let Err(e) = g.wait_healthy(&id, Duration::from_secs(10)).await {
                return err(StatusCode::GATEWAY_TIMEOUT, e.to_string());
            }
            Json(serde_json::json!({"ok": true})).into_response()
        }
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

async fn api_stop_region(State(sim): State<SharedSim>, Path(id): Path<String>) -> Response {
    let mut g = sim.lock().await;
    match g.stop_region(&id) {
        Ok(()) => Json(serde_json::json!({"ok": true})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

#[derive(Debug, Deserialize)]
struct StartWorkerBody {
    #[serde(default = "default_cap")]
    capacity: u32,
    #[serde(default = "default_bias")]
    kind_bias: String,
    #[serde(default = "default_work")]
    work_ms: u64,
}
fn default_cap() -> u32 {
    8
}
fn default_bias() -> String {
    "agent".into()
}
fn default_work() -> u64 {
    200
}

async fn api_start_worker(
    State(sim): State<SharedSim>,
    Path(id): Path<String>,
    Json(body): Json<StartWorkerBody>,
) -> Response {
    let mut g = sim.lock().await;
    match g.start_worker(&id, body.capacity, &body.kind_bias, body.work_ms) {
        Ok(wid) => Json(serde_json::json!({"ok": true, "worker_id": wid})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

async fn api_stop_worker(State(sim): State<SharedSim>, Path(id): Path<String>) -> Response {
    let mut g = sim.lock().await;
    match g.stop_worker(&id) {
        Ok(()) => Json(serde_json::json!({"ok": true})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

#[derive(Debug, Deserialize)]
struct SubmitBody {
    region_id: String,
    #[serde(default = "default_bias")]
    kind: String,
    #[serde(default = "default_units")]
    units: u32,
}
fn default_units() -> u32 {
    2
}

async fn api_submit(State(sim): State<SharedSim>, Json(body): Json<SubmitBody>) -> Response {
    let mut g = sim.lock().await;
    match g.submit_task(&body.region_id, &body.kind, body.units).await {
        Ok(resp) => Json(serde_json::json!({"ok": true, "response": resp.trim()})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

async fn api_stop_all(State(sim): State<SharedSim>) -> Response {
    let mut g = sim.lock().await;
    match g.stop_all() {
        Ok(()) => Json(serde_json::json!({"ok": true})).into_response(),
        Err(e) => err(StatusCode::BAD_REQUEST, e.to_string()),
    }
}

fn err(code: StatusCode, msg: String) -> Response {
    (code, Json(serde_json::json!({"ok": false, "error": msg}))).into_response()
}
