...
pub use request::{
    preflight_unsupported, CreateResponseRequest, RequestViolation, ResponseInput, Tool, ToolChoice,
    ToolChoiceMode, MAX_INSTRUCTIONS_BYTES, MAX_METADATA_ENTRIES, MAX_METADATA_KEY_BYTES,
...
pub const EXPLICITLY_UNSUPPORTED_FIELDS: &[(&str, &str)] = &[
    (
        "conversation",
        "server-side conversation objects are not supported; use previous_response_id to chain responses",
    ),
    (
        "context_management",
