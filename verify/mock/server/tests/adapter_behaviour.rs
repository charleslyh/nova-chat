...
use nova_responses::{
    AppendEvent, Attempt, ChainLimits, ContextError, ContextStore, EventBody, EventLogError,
    IdempotencyKey, NodeTag, ResponseEventKind, ResponseEventLog, ResponseId, ResponseItem,
...
async fn chain_resolves_in_chronological_order() {
    ...
    let encoded = nova_responses::canonical_items(&resolved.items[..1]);
    assert!(
...
async fn chain_never_includes_instructions() {
    ...
    let resolved = world
        ...
        .await
        .unwrap();
    let encoded = nova_responses::canonical_items(&resolved.items);
    assert!(
...
async fn idempotency_gate_has_no_ttl_window() {
    ...
    assert_eq!(
        replay,
        nova_responses::CreateOutcome::Duplicate { response_id }
    );
}
